# HertzScript Coroutine System Specification

The specification is currently a work-in-progress. For now, here's the high level description with examples!

See also:

- [HertzScript Coroutine Dispatcher](https://github.com/Floofies/hertzscript-dispatcher)
- [HertzScript Coroutine Compiler](https://github.com/Floofies/hertzscript-compiler)
...
[The HertzScript compiler](https://github.com/Floofies/hertzscript-compiler) uses a multi-stage source-to-source compilation pipeline to transform JavaScript functions into instruction streams; all functions within a HzScript program are GeneratorFunctions which yield instructions.
...
The Babel transformation plugin wraps all functions in calls detour functions, and the detour library is a list of function hooks designed to detour specific types of functions:
...
All hooks insert a detour which replaces the original function with a special invocation adapter, and upon invoking the adapter a new coroutine dispatcher is started in-place while the original function is run within it. The original function is assigned to a Symbol property named `tokenSym`. Each hook marks the detour and original function with special markers symbols which allow the coroutine dispatcher to observe information that would originally only be visible at compile-time.
...
To facilitate the aforementioned instruction stream transformations while preserving normal JavaScript execution, the [HertzScript coroutine dispatcher](https://github.com/Floofies/hertzscript-dispatcher) implements the instruction set and executes the compiled source code. The dispatcher consumes the instructions which are yielded by the compiled source code, performing the appropriate operations in response to each instruction.
...
The HertzScript instruction set is a list of "instruction tokens" and is composed of Kernelizer objects, which are generic objects designed to safely wrap user data. Some instruction tokens accept arguments in an array which are assigned to properties in itself via a `set` method. All tokens are gauranteed to only be in-use during 1 cycle because the coroutine dispatcher executes in a single thread, and so each instruction token is a single-instance uniqueness type to reduce memory overhead.

All instruction tokens are marked by having a special symbol assigned to them called `kernSym`, and if the coroutine dispatcher determines that an object has the symbol as a property then it assumes that the object is an instruction token and attempts to process it.
